...
if _lib_platform == 'darwin':
    _bitness = '64bit'
elif _lib_platform == 'win32':
...
if _dotraylib_used:
    try:
        ...
        _dotraylib_loadinfo = f"Platform ({_lib_platform}) and bitness ({_bitness}) not specified in .raylib file"

_lib_fname_abspath = os.path.normcase(os.path.normpath(_lib_default))

_cwd_info = (
    f"\n    current working dir: {os.getcwd()}" if _dotraylib_used else ""
)
_load_info = (
    f"\n    .raylib load info: {_dotraylib_loadinfo}"
    if _dotraylib_loadinfo
    else ""
)
...
class Color(Structure):
    ...
    def tint(self, tint):
        ...
        return _ColorTint(self, _color(tint))

    def brightness(self, factor):
        # type: (Color, float) -> Color
        """Get color with brightness correction, brightness factor goes from -1.0f to 1.0f"""
        return _ColorBrightness(self, float(factor))

    def contrast(self, contrast):
        # type: (Color, float) -> Color
        """Get color with contrast correction, contrast values between -1.0f and 1.0f"""
        return _ColorContrast(self, float(contrast))
...
class Rectangle(Structure):
    ...
    def __repr__(self):
        return f"Rectangle{self.__str__()}"
...
class Image(Structure):
    ...
    def gen_text(cls, width, height, text):
        ...
        return _GenImageText(int(width), int(height), _str_in(text))
